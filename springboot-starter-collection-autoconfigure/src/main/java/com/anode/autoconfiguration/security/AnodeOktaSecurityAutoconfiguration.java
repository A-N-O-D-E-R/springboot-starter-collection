...
@ConditionalOnProperty(value = "anode.security.env-okta", havingValue = "true")
@ConditionalOnDefaultWebSecurity
@AutoConfigureOrder(-1) // Permet de s'exécuter avant le starter okta et ne pas configurer leur SecurityFilterChain
public class AnodeOktaSecurityAutoconfiguration {

    @Bean
    public SecurityFilterChain configure(HttpSecurity http, Optional<ClientRegistrationRepository> clientRegistrationRepository, AnodeSecurityProperties anodeSecurityProperties, ClaimsToRoleMapper claimsToRoleMapper, List<Customizer<HttpSecurity>> httpSecurityCustomizers) throws Exception {
        ...
        if (anodeSecurityProperties.isMock()) {
            ...
            Okta.configureOAuth2WithPkce(http, clientRegistrationRepository.orElseThrow());
